...
We'll use one of the mapper's save events in order to send a notification e-mail to a predefined list of e-mail addresses. Events are great to completely decouple code. They also provide an extension point and code can be added without changing the original code by injecting features that are sometimes not even related to the main functionality. Also the decoupling is good because the code that saves the user message to the database should not know about what happens on such an event. You can add anything, from logging to email notifications or additional custom code that should run before or after a particular operation.

## Creating a notification mail service class

We will wrap the default mail service into a higher level service class which compose the mail message that needs to be sent on various notification ocasion. Create a new PHP class file in the `Service` folder
...
- we use our annotation service component to automatically inject dependencies
- we use the default mail service, which you already have configured if you are using the frontend application
- we inject a configuration key that we don't have it defined yet
...
This class is self-explanatory. We are using annotations to inject the one dependency needed - the notification mail service defined earlier. This class can listen to any mapper event. Just implement the ones you are interested in. In our case, the save event, we get the saved entity(check the dot-mapper documentation) and call the send notification method.
...
As you can see the options are defined per mapper/entity. The `event_listeners` key is used to attach you defined event listeners to the listener chain.

Test you code again. If you have configured the notification receivers you should receive a notification when a contact form is successfully submitted.
